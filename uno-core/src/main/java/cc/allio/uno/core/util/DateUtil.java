...
public class DateUtil {
    ...
    /**
     * 格式化小时数据集合 yyyy-MM-dd HH
     *
     * @return
     * @see #getHourDataSet()
     */
    public static Collection<String> getHourDataSetFormat() {
        return getHourDataSet()
                .stream()
                .map(date -> DateUtil.format(date, DateUtil.PATTERN_HOUR))
     ...
     * @return 28、30、31条数据
     */
    public static Collection<Date> getDayDataSet() {
        Date now = now();
        Calendar cal = Calendar.getInstance();
        cal.setTime(now);
    ...
    /**
     * 格式化当月天数数据 yyyy-MM-dd
     *
     * @return
     * @see #getDayDataSet()
     */
    public static Collection<String> getDayDataSetFormat() {
        return getDayDataSet()
                .stream()
                .map(day -> DateUtil.format(day, PATTERN_DATE))
     ...
     * @return 12条数据
     */
    public static Collection<Date> getMonthDataSet() {
        Date now = now();
        Calendar cal = Calendar.getInstance();
        cal.setTime(now);
        PriorityQueue<Date> dataSets = new PriorityQueue<>(Date::compareTo);
     ...
     * @see #getMonthDataSet()
     */
    public static Collection<String> getMonthDataSetFormat() {
        return getMonthDataSet()
                .stream()
                .map(month -> DateUtil.format(month, PATTERN_MONTH))
                .collect(Collectors.toList());
    }

    /**
     * 判断当前给定的时间是否在起始时间与结束时间范围内
     *
     * @param nowTime   当前给定时间
     * @param startTime 起始时间
     * @param endTime   结束时间
     * @return true 在当前时间内 false 不在当前时间内
     */
