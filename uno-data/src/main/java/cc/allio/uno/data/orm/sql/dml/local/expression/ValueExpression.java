...
public abstract class ValueExpression implements Expression {
    ...
    protected final String prefix = "context";
    ...
    @Override
    public Map<String, Object> getExpMapping() {
        Map<String, Object> exp = Maps.newHashMap();
        for (Tuple2<String, ExpressionValue> ex : valuePlaceholder.getList()) {
            exp.put(ex.getT1(), ex.getT2().getValue());
        }
        return exp;
    }
     ...
     * ValuePlaceholder实例
     */
    static class ValuePlaceholder implements OptionalContext {
        ...
        public List<Tuple2<String, ExpressionValue>> getList() {
            // 延迟加载
            if (CollectionUtils.isEmpty(placeholderAndValues)) {
