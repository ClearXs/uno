...
public interface WsEndpoint {
     ...
     * @throws NoSuchBeanDefinitionException 没有找到{@link WsProperties}的定义抛出
     * @throws NullPointerException          {@link BeanUtils#getContext()}为空时抛出
     */
    default WsProperties getProperties() {
        return BeanUtils.getBean(WsProperties.class);
    }
    ...
    default <T> Optional<String> combine(List<T> buffer) {
        ...
        return Optional.ofNullable(JsonUtils.getJsonMapper())
                .flatMap(objectMapper -> {
                    ...
                    for (T temp : buffer) {
                        String json;
                        if (temp instanceof String stringJson) {
                            json = stringJson;
                        } else {
                            json = JsonUtils.toJson(temp);
                        }
                        try {
                            JsonNode jsonNode = objectMapper.readTree(json);
                            if (jsonNode instanceof ArrayNode anode) {
                                arrayNode.addAll(anode);
                            } else {
