 ...
 * @since 1.0.6
 */
public class SrsMediaListCommand implements MediaListCommand {
    ...
    public SrsMediaListCommand(Integer start, Integer count) {
        this.start = start;
        this.count = count;
    }
    ...
    public Mono<CommandContext> execute(CommandContext context) throws NullPointerException, MediaException {
        ...
        return new SrsHttpResponseAdapter(
                HttpSwapper.build(srsProperty.getMediaUrl().concat("/api/v1/clients/"), HttpMethod.GET)
                        .addParameter("start", String.valueOf(1))
                        .addParameter("count", String.valueOf(Integer.MAX_VALUE))
                        .swap()
        )
