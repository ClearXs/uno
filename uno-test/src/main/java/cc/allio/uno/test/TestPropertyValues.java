...
public final class TestPropertyValues {
    ...
    private TestPropertyValues and(Stream<Pair> pairs) {
        Map<String, Object> copyProperties = new LinkedHashMap<>(this.properties);
        pairs.filter(Objects::nonNull).forEach(pair -> pair.addTo(copyProperties));
        return new TestPropertyValues(copyProperties);
    }
